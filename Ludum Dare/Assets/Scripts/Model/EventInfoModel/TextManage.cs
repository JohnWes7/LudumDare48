...
namespace FE_EventInfo
{
    public class TextManage : Single<TextManage>, IAddInfo
    {
        ...
        public TextManage()
        {
            string json = Tool.ReadTextFromResourcesJSON(Config.EventInfoTextPath); //获取json
            eventTextData = JsonConvert.DeserializeObject<EventTextData>(json);   //将json转成对应的类
        ...
        public static string GetText(string key, Config.Language language)
        {
            ...
            switch (language)
            {
                case Config.Language.l_simp_chinese:
                    if (Instance.eventTextData.l_simp_chinese != null)
                    {
                        success = Instance.eventTextData.l_simp_chinese.TryGetValue(key, out text);
                    }
                    break;
                case Config.Language.l_english:
                    if (Instance.eventTextData.l_english != null)
                    {
                        success = Instance.eventTextData.l_english.TryGetValue(key, out text);
                    }
                    break;
                default:
        ...
        public void AddInfo(string mod_directory_path)
        {
            ...
            if (!Tool.OpenDirectory(mod_directory_path, "/localisation", out directoryInfo))
            {
                return;
            }

            List<FileInfo> fileInfos = Tool.GetFiles(directoryInfo, "*.json");
            Debug.Log(directoryInfo.FullName + "中递归找到了" + fileInfos.Count + "个file");
